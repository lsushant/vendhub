package com.ibm.vendhub.web.rest;
...
import com.ibm.vendhub.IntegrationTest;
import com.ibm.vendhub.config.EmbeddedKafka;
import com.ibm.vendhub.config.KafkaSseConsumer;
import com.ibm.vendhub.config.KafkaSseProducer;
...
class VendhubKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/vendhub-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/vendhub-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/vendhub-kafka/unregister"));
                return;
            }
        }
